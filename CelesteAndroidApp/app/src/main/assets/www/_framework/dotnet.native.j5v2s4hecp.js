...
var INITIAL_MEMORY = Module["INITIAL_MEMORY"] || 58851328;
...
var ASM_CONSTS = {
 ...
 19295177: () => Module["SDL3"].audioContext.sampleRate,
 19295228: ($0, $1, $2, $3) => {
  var SDL3 = Module["SDL3"];
 ...
 19299541: ($0, $1) => {
  ...
  for (var c = 0; c < numChannels; ++c) {
   ...
   for (var j = 0; j < $1; ++j) {
    channelData[j] = GROWABLE_HEAP_F32()[buf + (j * numChannels + c) >>> 0];
   }
  }
 },
 19300074: ($0, $1) => {
 ...
 19301233: () => (navigator.mediaDevices === undefined) ? 0 : 1,
 19301292: ($0, $1, $2, $3, $4) => {
  const device = $0;
  const w = $1;
  const h = $2;
 ...
 19303849: ($0, $1, $2) => {
  ...
  GROWABLE_HEAP_U8().set(imgrgba, rgba >>> 0);
  return 1;
 },
 19304227: () => {
 ...
 19305354: ($0, $1, $2, $3) => {
  ...
  data32.set(GROWABLE_HEAP_I32().subarray(src >>> 0, src + data32.length >>> 0));
 ...
 19307601: () => Module["SDL3"]["mouse_x"],
 19307639: () => Module["SDL3"]["mouse_y"],
 19307677: $0 => Module["SDL3"]["mouse_buttons"][$0],
 19307725: $0 => {
  var data = $0;
  ...
  if (SDL3.makePointerEventCStruct === undefined) {
   SDL3.makePointerEventCStruct = function(left, top, event) {
    ...
    if (ptr != 0) {
     ...
     GROWABLE_HEAP_I32()[idx++ >>> 0] = ptrtype;
     GROWABLE_HEAP_I32()[idx++ >>> 0] = event.pointerId;
     GROWABLE_HEAP_I32()[idx++ >>> 0] = (typeof (event.button) !== "undefined") ? event.button : -1;
     GROWABLE_HEAP_I32()[idx++ >>> 0] = event.buttons;
     GROWABLE_HEAP_I32()[idx++ >>> 0] = (event.type == "pointerdown") ? 1 : 0;
     GROWABLE_HEAP_F32()[idx++ >>> 0] = event.movementX;
     GROWABLE_HEAP_F32()[idx++ >>> 0] = event.movementY;
     GROWABLE_HEAP_F32()[idx++ >>> 0] = event.clientX - left;
     GROWABLE_HEAP_F32()[idx++ >>> 0] = event.clientY - top;
     if (ptrtype == 3) {
      GROWABLE_HEAP_F32()[idx++ >>> 0] = event.pressure;
      GROWABLE_HEAP_F32()[idx++ >>> 0] = event.tangentialPressure;
      GROWABLE_HEAP_F32()[idx++ >>> 0] = event.tiltX;
      GROWABLE_HEAP_F32()[idx++ >>> 0] = event.tiltY;
      GROWABLE_HEAP_F32()[idx++ >>> 0] = event.twist;
     }
    }
    return ptr;
   };
 ...
 19309969: () => document.body.clientWidth,
 19310007: () => document.body.clientHeight,
 19310046: () => window.innerWidth,
 19310076: () => window.innerHeight,
 19310107: () => window.outerWidth,
 19310137: () => window.outerHeight,
 19310168: () => window.pageXOffset,
 19310199: () => window.pageYOffset,
 19310230: ($0, $1) => {
  ...
  if (target) {
   ...
   var makeDropEventCStruct = function(event) {
    ...
    if (ptr != 0) {
     var idx = ptr >> 2;
     var rect = target.getBoundingClientRect();
     GROWABLE_HEAP_I32()[idx++ >>> 0] = event.clientX - rect.left;
     GROWABLE_HEAP_I32()[idx++ >>> 0] = event.clientY - rect.top;
    }
    return ptr;
   };
 ...
 19316011: () => window.innerWidth,
 19316041: () => window.innerHeight,
 19316072: $0 => {
 ...
 19316181: ($0, $1) => {
  var pngData = GROWABLE_HEAP_U8().buffer instanceof ArrayBuffer ? GROWABLE_HEAP_U8().subarray($0 >>> 0, $0 + $1 >>> 0) : GROWABLE_HEAP_U8().slice($0, $0 + $1);
 ...
 19316748: () => window.innerWidth,
 19316778: () => window.innerHeight,
 19316809: $0 => {
 ...
 19320315: () => Module.window ? 1 : 0,
 19320349: () => {
  var win = Module.window ? Module.window : window;
 ...
 19320650: () => {
  ...
  Module.FMOD_JS_MixFunction = Module["cwrap"]("FMOD_JS_MixFunction", "void", [ "number" ]);
  return Module.context.sampleRate;
 },
 19320962: ($0, $1) => {
  Module._as_script_node = Module.context.createScriptProcessor($1, 0, $0);
  Module["OutputWebAudio_resumeAudio"] = function() {
   if (Module.context && Module.mInputRegistered) {
    ...
    Module._as_script_node.onaudioprocess = function(audioProcessingEvent) {
     Module._as_output_buffer = audioProcessingEvent.outputBuffer;
     Module.FMOD_JS_MixFunction(Module._as_output_buffer.getChannelData(0).length);
    };
    Module.context.resume();
    var win = Module.window ? Module.window : window;
    win.removeEventListener("click", Module.OutputWebAudio_resumeAudio, false);
    win.removeEventListener("touchend", Module.OutputWebAudio_resumeAudio, false);
    Module.mInputRegistered = false;
   }
  };
 },
 ...
 19322027: () => {
  var win = Module.window ? Module.window : window;
  win.addEventListener("touchend", Module["OutputWebAudio_resumeAudio"], false);
  win.addEventListener("click", Module["OutputWebAudio_resumeAudio"], false);
 ...
 19322335: ($0, $1, $2, $3) => {
  var data = GROWABLE_HEAP_F32().subarray(($0 / 4) >>> 0, ($0 / 4) + ($2 * $3) >>> 0);
 ...
 19322940: () => {
  ...
  Module.mContext.destination.channelCount = Module.mContext.destination.maxChannelCount;
 ...
 19323554: $0 => {
  ...
  Module.FMOD_JS_MixerSlowpathFunction = Module["cwrap"]("FMOD_JS_MixerSlowpathFunction", "void", []);
  Module.FMOD_JS_MixerFastpathFunction = Module["cwrap"]("FMOD_JS_MixerFastpathFunction", "void", [ "number" ]);
 ...
 19323988: ($0, $1) => {
  ...
  Module["OutputAudioWorklet_resumeAudio"] = function() {
   if (Module.mContext && Module.mInputRegistered) {
    ...
    var win = Module.window ? Module.window : window;
    win.removeEventListener("click", Module.OutputAudioWorklet_resumeAudio, false);
    win.removeEventListener("touchend", Module.OutputAudioWorklet_resumeAudio, false);
    Module.mInputRegistered = false;
    if (!Module.mModuleLoading) {
     Module.mModuleLoading = true;
     Module.mAddModuleRef++;
     Module.mContext.resume().then(function() {
      Module.mContext.audioWorklet.addModule(Module.mUrl).then(function() {
       if (Module.mAddModuleRef === 1) {
        Module.mWorkletNode = new AudioWorkletNode(Module.mContext, "audio-processor", {
         "outputChannelCount": [ Module.mSpeakerChannelCount ]
        });
        Module.mModulePolling = false;
        URL.revokeObjectURL(Module.mUrl);
        if (Module.mWorkletNode) {
         Module.mWorkletNode.port.postMessage(Module.mOutputData);
         if (Module.mSharedArrayBuffers) {
          Module.mWorkletNode.port.onmessage = function(event) {
         ...
         console.log("Error when creating AudioWorkletNode: Null object");
        }
       }
       Module.mAddModuleRef--;
      }).catch(function(err) {
 ...
 19329643: () => {
  var win = Module.window ? Module.window : window;
  win.addEventListener("touchend", Module["OutputAudioWorklet_resumeAudio"], false);
  win.addEventListener("click", Module["OutputAudioWorklet_resumeAudio"], false);
  Module.mInputRegistered = true;
 },
 19329892: () => {
 ...
 19330612: () => {
  Module.mStartInterval = Module["waitForAudioWorklet"](function() {
 ...
 19330889: () => {
  if (Module["waitForAudioWorklet"]) {
   Module.mStopInterval = Module["waitForAudioWorklet"](function() {
    return (Module.mWorkletNode === null && Module.mModulePolling || !Module.mWorkletNodeConnected);
   }, function() {
    if (Module.mWorkletNode) {
     Module.mWorkletNode.disconnect();
     Module.mWorkletNodeConnected = false;
 ...
 19331214: ($0, $1, $2) => {
  ...
  var data = GROWABLE_HEAP_F32().subarray((buffer / 4) >>> 0, (buffer / 4) + (bufferLength * speakerModeChannels) >>> 0);
  for (var channel = 0; channel < speakerModeChannels; channel++) {
   ...
   for (var sample = 0; sample < bufferLength; sample++) {
    Module.mOutputData[sample + offset] = data[(sample * speakerModeChannels) + channel];
   }
  }
 },
 19331641: ($0, $1, $2, $3) => {
...
var __wasmfs_create_js_file_backend_js = function(backend) {
 backend >>>= 0;
 wasmFS$backends[backend] = {
  ...
  write: (file, buffer, length, offset) => {
   try {
    ...
    wasmFS$JSMemoryFiles[file].set(GROWABLE_HEAP_U8().subarray(buffer >>> 0, buffer + length >>> 0), offset);
    return length;
   } catch (err) {
    return -29;
   }
  ...
  read: (file, buffer, length, offset) => {
   ...
   GROWABLE_HEAP_U8().set(fileData.subarray(offset, offset + length), buffer >>> 0);
   return length;
  },
...
async function __wasmfs_jsimpl_async_read(ctx, backend, file, buffer, length, offset, result_p) {
 ...
 GROWABLE_HEAP_U32()[((result_p) >>> 2) >>> 0] = result;
 _emscripten_proxy_finish(ctx);
}
...
var fillBatteryEventData = (eventStruct, e) => {
 GROWABLE_HEAP_F64()[((eventStruct) >>> 3) >>> 0] = e.chargingTime;
 GROWABLE_HEAP_F64()[(((eventStruct) + (8)) >>> 3) >>> 0] = e.dischargingTime;
 GROWABLE_HEAP_F64()[(((eventStruct) + (16)) >>> 3) >>> 0] = e.level;
 GROWABLE_HEAP_I32()[(((eventStruct) + (24)) >>> 2) >>> 0] = e.charging;
};
...
function _emscripten_get_fullscreen_status(fullscreenStatus) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(12, 0, 1, fullscreenStatus);
 fullscreenStatus >>>= 0;
 if (!JSEvents.fullscreenEnabled()) return -1;
...
function _emscripten_get_gamepad_status(index, gamepadState) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(13, 0, 1, index, gamepadState);
 gamepadState >>>= 0;
...
function _emscripten_get_main_loop_timing(mode, value) {
 mode >>>= 0;
 value >>>= 0;
 if (mode) GROWABLE_HEAP_I32()[((mode) >>> 2) >>> 0] = Browser.mainLoop.timingMode;
 if (value) GROWABLE_HEAP_I32()[((value) >>> 2) >>> 0] = Browser.mainLoop.timingValue;
}
...
function _emscripten_get_screen_size(width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(15, 0, 1, width, height);
 width >>>= 0;
 height >>>= 0;
...
function _emscripten_request_fullscreen_strategy(target, deferUntilInEventHandler, fullscreenStrategy) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(16, 0, 1, target, deferUntilInEventHandler, fullscreenStrategy);
 target >>>= 0;
...
function _emscripten_request_pointerlock(target, deferUntilInEventHandler) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(17, 0, 1, target, deferUntilInEventHandler);
 target >>>= 0;
 target = findEventTarget(target);
 if (!target) return -4;
 if (!target.requestPointerLock) {
...
function _emscripten_set_beforeunload_callback_on_thread(userData, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(19, 0, 1, userData, callbackfunc, targetThread);
 userData >>>= 0;
 callbackfunc >>>= 0;
 targetThread >>>= 0;
...
var fillDeviceMotionEventData = (eventStruct, e, target) => {
 ...
 ag = ag || {};
 rr = rr || {};
 GROWABLE_HEAP_F64()[((eventStruct) >>> 3) >>> 0] = a["x"];
 GROWABLE_HEAP_F64()[(((eventStruct) + (8)) >>> 3) >>> 0] = a["y"];
 GROWABLE_HEAP_F64()[(((eventStruct) + (16)) >>> 3) >>> 0] = a["z"];
 GROWABLE_HEAP_F64()[(((eventStruct) + (24)) >>> 3) >>> 0] = ag["x"];
 GROWABLE_HEAP_F64()[(((eventStruct) + (32)) >>> 3) >>> 0] = ag["y"];
 GROWABLE_HEAP_F64()[(((eventStruct) + (40)) >>> 3) >>> 0] = ag["z"];
 GROWABLE_HEAP_F64()[(((eventStruct) + (48)) >>> 3) >>> 0] = rr["alpha"];
 GROWABLE_HEAP_F64()[(((eventStruct) + (56)) >>> 3) >>> 0] = rr["beta"];
 GROWABLE_HEAP_F64()[(((eventStruct) + (64)) >>> 3) >>> 0] = rr["gamma"];
};
...
function _emscripten_set_devicemotion_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(21, 0, 1, userData, useCapture, callbackfunc, targetThread);
 userData >>>= 0;
 callbackfunc >>>= 0;
 targetThread >>>= 0;
...
var fillOrientationChangeEventData = eventStruct => {
 ...
 GROWABLE_HEAP_I32()[((eventStruct) >>> 2) >>> 0] = orientationIndex;
 GROWABLE_HEAP_I32()[(((eventStruct) + (4)) >>> 2) >>> 0] = orientationAngle;
};
...
function _emscripten_set_window_title(title) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 0, 1, title);
 title >>>= 0;
...
var FS_writeFile = (path, data) => withStackSave(() => {
 ...
 for (var i = 0; i < data.length; i++) {
  GROWABLE_HEAP_I8()[(dataBuffer) + (i) >>> 0] = data[i];
 }
...
var FS = {
 ...
 readFile(path, opts = {}) {
  ...
  var ret = new Uint8Array(GROWABLE_HEAP_U8().subarray(buf + 8 >>> 0, buf + 8 + length >>> 0));
 ...
 read(stream, buffer, offset, length, position) {
  ...
  for (var i = 0; i < length; i++) {
   buffer[offset + i] = GROWABLE_HEAP_I8()[(dataBuffer) + (i) >>> 0];
  }
  _free(dataBuffer);
  return bytesRead;
 },
 write(stream, buffer, offset, length, position, canOwn) {
  ...
  var dataBuffer = _malloc(length);
  for (var i = 0; i < length; i++) {
   GROWABLE_HEAP_I8()[(dataBuffer) + (i) >>> 0] = buffer[offset + i];
  }
  var bytesRead;
  if (seeking) {
 ...
 statBufToObject(statBuf) {
  return {
   dev: GROWABLE_HEAP_U32()[((statBuf) >>> 2) >>> 0],
   mode: GROWABLE_HEAP_U32()[(((statBuf) + (4)) >>> 2) >>> 0],
   nlink: GROWABLE_HEAP_U32()[(((statBuf) + (8)) >>> 2) >>> 0],
   uid: GROWABLE_HEAP_U32()[(((statBuf) + (12)) >>> 2) >>> 0],
   gid: GROWABLE_HEAP_U32()[(((statBuf) + (16)) >>> 2) >>> 0],
   rdev: GROWABLE_HEAP_U32()[(((statBuf) + (20)) >>> 2) >>> 0],
   size: readI53FromI64((statBuf) + (24)),
   blksize: GROWABLE_HEAP_U32()[(((statBuf) + (32)) >>> 2) >>> 0],
   blocks: GROWABLE_HEAP_U32()[(((statBuf) + (36)) >>> 2) >>> 0],
...
var _FMOD_JS_MixFunction = Module["_FMOD_JS_MixFunction"] = a0 => (_FMOD_JS_MixFunction = Module["_FMOD_JS_MixFunction"] = wasmExports["FMOD_JS_MixFunction"])(a0);

var _FMOD_JS_MixerSlowpathFunction = Module["_FMOD_JS_MixerSlowpathFunction"] = () => (_FMOD_JS_MixerSlowpathFunction = Module["_FMOD_JS_MixerSlowpathFunction"] = wasmExports["FMOD_JS_MixerSlowpathFunction"])();

var _FMOD_JS_MixerFastpathFunction = Module["_FMOD_JS_MixerFastpathFunction"] = a0 => (_FMOD_JS_MixerFastpathFunction = Module["_FMOD_JS_MixerFastpathFunction"] = wasmExports["FMOD_JS_MixerFastpathFunction"])(a0);
...
var _perform_thread_dump = Module["_perform_thread_dump"] = () => (_perform_thread_dump = Module["_perform_thread_dump"] = wasmExports["perform_thread_dump"])();
...
var _mono_aot_CelesteLoader_get_method = Module["_mono_aot_CelesteLoader_get_method"] = a0 => (_mono_aot_CelesteLoader_get_method = Module["_mono_aot_CelesteLoader_get_method"] = wasmExports["mono_aot_CelesteLoader_get_method"])(a0);
...
var _mono_aot_KeraLua_get_method = Module["_mono_aot_KeraLua_get_method"] = a0 => (_mono_aot_KeraLua_get_method = Module["_mono_aot_KeraLua_get_method"] = wasmExports["mono_aot_KeraLua_get_method"])(a0);

var _mono_aot_Mono_Cecil_Mdb_get_method = Module["_mono_aot_Mono_Cecil_Mdb_get_method"] = a0 => (_mono_aot_Mono_Cecil_Mdb_get_method = Module["_mono_aot_Mono_Cecil_Mdb_get_method"] = wasmExports["mono_aot_Mono_Cecil_Mdb_get_method"])(a0);

var _mono_aot_Mono_Cecil_Pdb_get_method = Module["_mono_aot_Mono_Cecil_Pdb_get_method"] = a0 => (_mono_aot_Mono_Cecil_Pdb_get_method = Module["_mono_aot_Mono_Cecil_Pdb_get_method"] = wasmExports["mono_aot_Mono_Cecil_Pdb_get_method"])(a0);

var _mono_aot_Mono_Cecil_Rocks_get_method = Module["_mono_aot_Mono_Cecil_Rocks_get_method"] = a0 => (_mono_aot_Mono_Cecil_Rocks_get_method = Module["_mono_aot_Mono_Cecil_Rocks_get_method"] = wasmExports["mono_aot_Mono_Cecil_Rocks_get_method"])(a0);

var _mono_aot_Mono_Cecil_get_method = Module["_mono_aot_Mono_Cecil_get_method"] = a0 => (_mono_aot_Mono_Cecil_get_method = Module["_mono_aot_Mono_Cecil_get_method"] = wasmExports["mono_aot_Mono_Cecil_get_method"])(a0);
...
var _mono_aot_Celeste_Wasm_mm_get_method = Module["_mono_aot_Celeste_Wasm_mm_get_method"] = a0 => (_mono_aot_Celeste_Wasm_mm_get_method = Module["_mono_aot_Celeste_Wasm_mm_get_method"] = wasmExports["mono_aot_Celeste_Wasm_mm_get_method"])(a0);
...
var _mono_aot_MonoMod_Core_get_method = Module["_mono_aot_MonoMod_Core_get_method"] = a0 => (_mono_aot_MonoMod_Core_get_method = Module["_mono_aot_MonoMod_Core_get_method"] = wasmExports["mono_aot_MonoMod_Core_get_method"])(a0);

var _mono_aot_MonoMod_ILHelpers_get_method = Module["_mono_aot_MonoMod_ILHelpers_get_method"] = a0 => (_mono_aot_MonoMod_ILHelpers_get_method = Module["_mono_aot_MonoMod_ILHelpers_get_method"] = wasmExports["mono_aot_MonoMod_ILHelpers_get_method"])(a0);

var _mono_aot_MonoMod_Patcher_get_method = Module["_mono_aot_MonoMod_Patcher_get_method"] = a0 => (_mono_aot_MonoMod_Patcher_get_method = Module["_mono_aot_MonoMod_Patcher_get_method"] = wasmExports["mono_aot_MonoMod_Patcher_get_method"])(a0);

var _mono_aot_MonoMod_RuntimeDetour_get_method = Module["_mono_aot_MonoMod_RuntimeDetour_get_method"] = a0 => (_mono_aot_MonoMod_RuntimeDetour_get_method = Module["_mono_aot_MonoMod_RuntimeDetour_get_method"] = wasmExports["mono_aot_MonoMod_RuntimeDetour_get_method"])(a0);

var _mono_aot_MonoMod_RuntimeDetour_HookGen_get_method = Module["_mono_aot_MonoMod_RuntimeDetour_HookGen_get_method"] = a0 => (_mono_aot_MonoMod_RuntimeDetour_HookGen_get_method = Module["_mono_aot_MonoMod_RuntimeDetour_HookGen_get_method"] = wasmExports["mono_aot_MonoMod_RuntimeDetour_HookGen_get_method"])(a0);
...
var ___start_em_js = Module["___start_em_js"] = 19333720;

var ___stop_em_js = Module["___stop_em_js"] = 19334812;
